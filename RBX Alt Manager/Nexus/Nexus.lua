...
do -- Nexus
	...
	function Nexus:AddCommand(Name, Function)
		self.Commands[string.lower(Name)] = Function
	end
	...
	Nexus.MessageReceived:Connect(function(Message)
		...
		elseif Nexus.Commands[string.lower(Message)] then
			local Success, Error = pcall(Nexus.Commands[string.lower(Message)], Message)
...
do -- Default Commands
	...
	Nexus:AddCommand('performance', function(Message)		
		...
		if not _TARGETFPS then _TARGETFPS = 8 end
		...
		repeat task.wait(1) until iswindowactive()
		RunService:Set3dRenderingEnabled(true)
		settings().Rendering.QualityLevel = OldLevel
		setfpscap(60)
	end)
